...
namespace Tools
{
    ...
    public static class CameraPlane
    {
        ...
        {
            Vector3 point = Camera.main.ScreenToViewportPoint(screenCoord);
        ...
        /// </summary>
        public static Vector2 ViewportPointToAngle(Camera cam, Vector2 viewportCoords)
        {
            float adjustedAngle = AngleProportion(cam.fieldOfView / 2, cam.aspect) * 2;
            float xProportion = (viewportCoords.x - .5f) / .5f;
            float yProportion = (viewportCoords.y - .5f) / .5f;
